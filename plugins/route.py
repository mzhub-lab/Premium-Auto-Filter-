...
async def verify_redirect_handler(request: web.Request):
    ...
    try:
        # Safe split to handle underscores in file_id
        token_parts = token.split("_", 3)
        user_id = int(token_parts[1]) if len(token_parts) > 1 else 0
        ...
        grp_int_id = int(grp_id) if grp_id and str(grp_id) != "0" else 0

        # Agar grp_id 0 hai (PM search/verify), toh direct info.py ke defaults use honge
        if grp_int_id == 0:
            ...
            try:
                short_url = await shortzy.convert(target_tg_link)
            except Exception:
                short_url = await shortzy.get_quick_link(target_tg_link)
        else:
        ...
        raise redirect
    except web.HTTPMovePermanently as redirect:
        raise redirect
