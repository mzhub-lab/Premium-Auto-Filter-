...
async def queue_movie_file(bot, media):
    try:
        ...
        # Clean grouping title: Show name nikalna bina episode data delete kiye
        clean_title = file_name
        season_match = re.search(r"(?i)\b(?:s|season)\s*0*(\d{1,2})\b", file_name)
        if season_match:
            clean_title = file_name[: season_match.start()].strip()
        elif year:
        ...
        group_key = clean_title or file_name
...
async def send_movie_update(bot, file_name, files):
    ...
    try:
        ...
        # Regex for Combined/Range episodes (E01-E45, [E01-E45], S01 [E01-E100])
        combined_pattern = re.compile(
            r"(?:S(\d{1,2}))?[\s_\[\-(]*E(?:P)?(\d{1,4})\s*[-~to]+\s*E?(?:P)?(\d{1,4})[\]\)\s_]*",
            re.IGNORECASE,
        )
        episode_pattern = re.compile(
            r"S(\d{1,2})[\s_\[\-(]*E(?:P)?(\d{1,4})|(?<![a-z0-9])E(?:P)?(\d{1,4})(?![a-z0-9])",
            re.IGNORECASE,
        )
        ...
        for file in files:
            ...
            # Priority: Raw original filename pehle check karenge taaki brackets intact milein
            ...
            # STEP 1: Pehle check karenge ki Combined/Range hai ya nahi
            if combined_match:
            ...
            # STEP 2: Agar combined nahi hai, tab Single Episode banega
            elif match:
                s_val = int(match.group(1)) if match.group(1) else 1
                e_val = int(match.group(2)) if match.group(2) else int(match.group(3))
                ep = f"S{s_val:02d}E{e_val:02d}"
                episode_map[ep][quality] = file
            # STEP 3: Fallback batch
