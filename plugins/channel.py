...
# FIX: Title ke liye single {} rakha hai taaki dynamic text pass ho sake
...
async def send_movie_update(bot, file_name, files):
    try:
        ...
        # Safe year parsing taaki 'None' string mix na ho
        ...
        for file in files:
            ...
            if match:
                ...
                # FIX: Combined links text changed to '📥 Download'
                combined_links.append(f"📦 {ep} ({quality}) : <a href='https://t.me/{temp.U_NAME}?start=file_0_{file_id}'>📥 Download</a>")
            elif re.search(r"complete|completed|batch|combined", caption, re.IGNORECASE):
                # FIX: Batch link text changed to '📥 Download'
                combined_links.append(f"📦 ({quality}) : <a href='https://t.me/{temp.U_NAME}?start=file_0_{file_id}'>📥 Download</a>")
        ...
        for ep, qualities in sorted(episode_map.items()):
            ...
            for quality in sorted(qualities.keys()):
                f = qualities[quality]
                # FIX: Series buttons text changed to '📥 Download (Quality)' taaki text standard rahe
                link = f"<a href='https://t.me/{temp.U_NAME}?start=file_0_{f['file_id']}'>📥 Download ({quality})</a>"
                parts.append(link)
            joined = " - ".join(parts)
        ...
        if not quality_text:
            ...
            for quality, q_files in sorted(quality_groups.items()):
                # FIX: Normal movie hyperlink text changed from Size to '📥 Download'
                links = [f"<a href='https://t.me/{temp.U_NAME}?start=file_0_{f['file_id']}'>📥 Download</a>" for f in q_files]
                line = f"📦 {quality} : " + " | ".join(links)
                quality_text += line + "\n"

        image_url = poster or "https://te.legra.ph/file/88d845b4f8a024a71465d.jpg"
        
        # FIX: Agar year present hai toh ' (2026)' brackets me judega, warna clean khali rahega
        clean_year = f" ({year})" if year else ""
        final_title = f"{title}{clean_year}"
        
        # Template format matches perfectly (5 elements total)
        full_caption = UPDATE_CAPTION.format(kind, final_title, files[0]['quality'], language, quality_text)
